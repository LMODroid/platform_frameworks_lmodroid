...
package com.libremobileos.server.app;
...
public final class ParallelSpaceManagerService extends SystemService {
    ...
    private void broadcastChange() {
        ...
        Intent intent = new Intent(com.libremobileos.content.Intent.ACTION_PARALLEL_SPACE_CHANGED);
        intent.addFlags(Intent.FLAG_RECEIVER_INCLUDE_BACKGROUND);
        getContext().sendBroadcastAsUser(intent, UserHandle.of(userId),
                com.libremobileos.Manifest.permission.MANAGE_PARALLEL_SPACES);
    }
    ...
    public void onStart() {
        ...
        publishBinderService(LMOContextConstants.PARALLEL_SERVICE, mInterface);
        LocalServices.addService(ParallelSpaceManagerServiceInternal.class, new LocalService());
    }
    ...
    private void enforceCallingPermission(int uid) {
        if (uid == Process.SYSTEM_UID || uid == Process.SHELL_UID)
            return;
        if (!hasPermissionGranted(com.libremobileos.Manifest.permission.MANAGE_PARALLEL_SPACES, uid))
            throw new SecurityException("Caller does not have permission MANAGE_PARALLEL_SPACES");
    }

    /**
     * Class that act as system server internal api. They can only be
     * accessed within system server process.
     */
    private class LocalService implements ParallelSpaceManagerServiceInternal {

        @Override
        public int convertToParallelOwnerIfPossible(int userId) {
            synchronized (mLock) {
                if (mCurrentParallelUserIds != null && mCurrentParallelUserIds.contains(userId))
        ...
        @Override
        public boolean isCurrentParallelUser(int userId) {
            synchronized (mLock) {
                if (mCurrentParallelUserIds != null)
        ...
        @Override
        public int getCurrentParallelOwnerId() {
            synchronized (mLock) {
                return mCurrentUserId;
            }
        }
        ...
        @Override
        public boolean isCurrentParallelOwner(int userId) {
            return userId == getCurrentParallelOwnerId();
        }

        @Override
        public List<Integer> getCurrentParallelUserIds() {
        ...
        @Override
        public boolean isInteractive(int userId) {
            synchronized (mLock) {
        ...
        @Override
        public boolean canInteract(int userId1, int userId2) {
            return isInteractive(userId1) && isInteractive(userId2);
        }

        @Override
        public List<UserInfo> getInteractiveUsers() {
